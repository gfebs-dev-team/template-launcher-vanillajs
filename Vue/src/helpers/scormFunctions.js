...
function Initialize() {
	...
	scorm.storeDataValue( "cmi.exit","suspend" );
...
function Terminate() {
    if(scorm.retrieveDataValue("cmi.completion_status") == "incomplete") {
        scorm.storeDataValue("cmi.suspend_data", "suspended");
		scorm.storeDataValue("cmi.success_status", "unknown");
		scorm.storeDataValue("adl.nav.request", "suspendAll");
		scorm.persistData();
    } else {
		scorm.storeDataValue("cmi.success_status", "passed");
	}
...
export function doExit() {
	...
	scorm.storeDataValue("cmi.session_time", scormTime);
	scorm.storeDataValue("cmi.exit", "normal");
	scorm.storeDataValue("adl.nav.request", "exitAll");
...
function SetIncomplete (){
	const status = scorm.retrieveDataValue( "cmi.completion_status" );
	if (status != "completed"){
		scorm.storeDataValue( "cmi.completion_status", "incomplete" );
	}
}
...
function SetComplete (){
	scorm.storeDataValue( "cmi.completion_status", "completed" );
